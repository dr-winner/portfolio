      ...
      >
        <Command label="Command menu" className="flex flex-col">
          ...
          <Command.List className="max-h-[min(60vh,520px)] overflow-y-auto p-2 sm:max-h-[60vh]">
            <Command.Empty className="py-6 text-center text-sm text-slate-500 dark:text-white/65">
              No matches.
            </Command.Empty>
            {Object.entries(grouped).map(([group, list]) => (
              <Command.Group
                key={group}
                heading={group}
                className="mb-1 [&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:pb-1 [&_[cmdk-group-heading]]:pt-3 [&_[cmdk-group-heading]]:text-[11px] [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-[0.2em] [&_[cmdk-group-heading]]:text-slate-500 [&_[cmdk-group-heading]]:dark:text-white/55"
              >
                {list.map((it) => {
                  const Icon = it.icon;
                  return (
          ...
          <div className="flex items-center justify-between border-t border-slate-200/90 px-4 py-2 text-[11px] text-slate-500 dark:border-white/10 dark:text-white/55">
