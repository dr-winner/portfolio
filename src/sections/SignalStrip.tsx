  ...
  return (
    <section aria-label="Signal keywords" className="relative pb-8 md:pb-10">
      <div className="container">
        <div className="w-full max-w-5xl mx-auto rounded-2xl border border-white/10 bg-ink-100/55 py-0 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.05)]">
                ...
                {[...Array(2)].map((_, i) => (
                  <Fragment key={i}>
                    {signalWords.map((w) => (
                      <span
                        key={`${i}-${w}`}
                        className="inline-flex shrink-0 items-center gap-2.5 text-[12px] md:text-[13px] tracking-tight text-white/50"
                      >
                        <span className="size-1 rounded-full bg-cyber-300/50 shadow-glow-sm" />
